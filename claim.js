...
const TRANSACTION_LIMIT_PER_SECOND = 10;
...
async function doTransactions(tx, keypair, retries = 100) {
  const currentTime = Date.now();

  // Wait if necessary to respect the rate limit
  const timeSinceLastTransaction = currentTime - lastTransactionTime;
  if (timeSinceLastTransaction < 100 / TRANSACTION_LIMIT_PER_SECOND) {
    await new Promise((resolve) => setTimeout(resolve, (100 / TRANSACTION_LIMIT_PER_SECOND) - timeSinceLastTransaction));
  }
  ...
  try {
    ...
    const signature = await connection.sendRawTransaction(bufferTransaction, {
      ...
      computeUnitLimit: 2000000, // Adjust based on your needs
    });

    await connection.confirmTransaction(signature);
    lastTransactionTime = Date.now(); // Update last transaction time
    return signature;
  } catch (error) {
...
async function openMysteryBox(token, keypair, retries = 100) {
  try {
    const { data } = await axios({
      url:
        apiBaseUrl +
    ...
    if (retries > 0) {
      ...
      console.log(`Error details: ${JSON.stringify(error, null, 2)}`.red);
      throw error;
    }
  }
}
